...
function councilMembers(db: DB, councilId: string) {
  return (
    db
      ...
      .all(councilId) as never[]
  ) as ReturnType<typeof memberToDTO>[]
}
...
export function registerMemberCouncilRoutes(app: FastifyInstance, db: DB): void {
  ...
  app.patch('/api/v1/members/:id', async (req) => {
    ...
    const c = cur as { name: string; model_id: string; system_prompt: string | null; temperature: number; max_tokens: number | null; avatar_color: string; enabled: number }
    db.prepare(
  ...
  app.patch('/api/v1/councils/:id', async (req) => {
    ...
    const c = cur as { name: string; description: string | null; strategy: string; rounds: number; moderator_member_id: string | null }
