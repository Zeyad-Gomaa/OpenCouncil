...
var init_schemas = __esm({
  "packages/shared/dist/schemas.js"() {
    "use strict";
    providerProtocolSchema = z4.enum(["openai_compatible", "anthropic", "google", "mock"]);
    providerCreateSchema = z4.object({
      name: z4.string().min(1).max(80),
      protocol: providerProtocolSchema,
      baseUrl: z4.string().url().optional(),
      apiKey: z4.string().max(4096).optional(),
      defaultModelId: z4.string().max(200).nullish(),
      enabled: z4.boolean().optional()
    });
    providerUpdateSchema = z4.object({
      name: z4.string().min(1).max(80).optional(),
      protocol: providerProtocolSchema.optional(),
      baseUrl: z4.string().url().nullable().optional(),
      apiKey: z4.string().max(4096).nullable().optional(),
      defaultModelId: z4.string().max(200).nullable().optional(),
      enabled: z4.boolean().optional()
    });
    modelCreateSchema = z4.object({
      providerId: z4.string().uuid(),
      modelId: z4.string().min(1).max(200),
      displayName: z4.string().min(1).max(120),
      contextWindow: z4.number().int().positive().max(1e8).nullish(),
      inputPerMTokUsd: z4.number().nonnegative().nullish(),
      outputPerMTokUsd: z4.number().nonnegative().nullish(),
      enabled: z4.boolean().optional()
    });
    modelUpdateSchema = modelCreateSchema.partial().omit({ providerId: true });
    modelBatchUpdateSchema = z4.object({
      modelIds: z4.array(z4.string().uuid()).min(1).max(500),
      patch: modelUpdateSchema.refine((value) => Object.keys(value).length > 0, "patch must change at least one field")
    });
    memberBatchModelSchema = z4.object({
      memberIds: z4.array(z4.string().uuid()).min(1).max(500),
      modelId: z4.string().uuid(),
      maxTokens: z4.number().int().positive().max(2e5).nullish()
    });
    catalogEnrollSchema = z4.object({
      modelIds: z4.array(z4.string().min(1).max(200)).min(1).max(500)
    });
    memberCreateSchema = z4.object({
      name: z4.string().min(1).max(60),
      modelId: z4.string().uuid(),
      systemPrompt: z4.string().max(2e4).nullish(),
      temperature: z4.number().min(0).max(2).optional(),
      maxTokens: z4.number().int().positive().max(2e5).nullish(),
      avatarColor: z4.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),
      enabled: z4.boolean().optional()
    });
    memberUpdateSchema = memberCreateSchema.partial();
    strategyKindSchema = z4.enum([
      "round_robin",
      "debate",
      "swarm",
      "critique",
    ...
    councilCreateSchema = z4.object({
      name: z4.string().min(1).max(80),
      description: z4.string().max(500).nullish(),
      strategy: strategyKindSchema,
      rounds: z4.number().int().min(1).max(100),
      memberIds: z4.array(z4.string().uuid()).min(1).max(24),
      moderatorMemberId: z4.string().uuid().nullish()
    ...
    councilUpdateSchema = z4.object({
      name: z4.string().min(1).max(80).optional(),
      description: z4.string().max(500).nullable().optional(),
      strategy: strategyKindSchema.optional(),
      rounds: z4.number().int().min(1).max(100).optional(),
      memberIds: z4.array(z4.string().uuid()).min(1).max(24).optional(),
      moderatorMemberId: z4.string().uuid().nullable().optional()
    }).refine((c) => !c.moderatorMemberId || (c.memberIds ? c.memberIds.includes(c.moderatorMemberId) : true), {
      message: "moderator must be one of the council members"
    });
    sessionCreateSchema = z4.object({
      councilId: z4.string().uuid(),
      topic: z4.string().trim().min(1).max(8e3),
      researchEnabled: z4.boolean().optional(),
      budgetUsd: z4.number().positive().finite().max(1e5).optional(),
      consensusEnabled: z4.boolean().optional(),
      workspacePath: z4.string().min(1).max(4e3).optional(),
      workspaceFiles: z4.array(z4.string().min(1).max(1e3)).max(80).optional()
    });
    workspacePreviewSchema = z4.object({
      path: z4.string().min(1).max(4e3),
      files: z4.array(z4.string().min(1).max(1e3)).max(80).optional()
    });
    sessionExtendSchema = z4.object({
      additionalRounds: z4.number().int().min(1).max(50).default(1)
    });
    sessionConcludeSchema = z4.object({
      reason: z4.string().max(500).optional()
    });
    sessionInterveneSchema = z4.object({
      content: z4.string().min(1).max(4e3)
    });
    configImportSchema = z4.object({
      version: z4.literal(1).optional(),
      providers: z4.array(z4.object({
        id: z4.string().uuid(),
        name: z4.string().min(1).max(80),
        protocol: providerProtocolSchema,
        baseUrl: z4.string().url().nullish(),
        defaultModelId: z4.string().max(200).nullish(),
        enabled: z4.coerce.boolean().optional()
      })),
      models: z4.array(z4.object({
        id: z4.string().uuid(),
        providerId: z4.string().uuid(),
        modelId: z4.string().min(1).max(200),
        displayName: z4.string().min(1).max(120),
      ...
      members: z4.array(z4.object({
        id: z4.string().uuid(),
        name: z4.string().min(1).max(60),
        modelId: z4.string().uuid().nullish(),
        systemPrompt: z4.string().max(2e4).nullish(),
        temperature: z4.number().min(0).max(2).optional(),
        maxTokens: z4.number().int().positive().max(2e5).nullish(),
        avatarColor: z4.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),
      ...
      councils: z4.array(z4.object({
        id: z4.string().uuid(),
        name: z4.string().min(1).max(80),
        description: z4.string().max(500).nullish(),
        strategy: strategyKindSchema.optional(),
        rounds: z4.number().int().min(1).max(100).optional(),
        memberIds: z4.array(z4.string().uuid()).max(24).optional(),
        moderatorMemberId: z4.string().uuid().nullish()
      }))
    });
  }
...
var init_activity = __esm({
  "apps/server/src/routes/activity.ts"() {
    "use strict";
    windowSchema = z5.object({ days: z5.coerce.number().int().min(1).max(365).default(30) });
  }
});
...
var envSchema = z.object({
  ...
  OPEN_COUNCIL_SECRET_KEY: z.preprocess((value) => value === "" ? void 0 : value, z.string().min(8).optional()),
  OPEN_COUNCIL_OPERATOR_TOKEN: z.preprocess((v) => v === "" ? void 0 : v, z.string().min(32).max(4096).optional()),
  ...
  WEB_RESEARCH_ENABLED: z.enum(["true", "false", "1", "0"]).default("true").transform((v) => v === "true" || v === "1")
});
...
var ballotSchema = z2.object({
  ranking: z2.array(z2.string()).min(2).max(24),
  rationale: z2.string().min(1).max(4e3)
}).strict();
...
function registerOperatorAuth(app, config) {
  ...
  app.post("/api/v1/auth/login", async (req, reply) => {
    ...
    const { token } = z3.object({ token: z3.string().min(1).max(4096) }).parse(req.body);
