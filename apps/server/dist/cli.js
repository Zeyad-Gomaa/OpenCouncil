...
async function buildApp(deps) {
  ...
  app.get("/api/v1/health", async () => ({ ok: true, version: "0.1.0", instanceId: randomUUID6() }));
...
function parseArgs(argv) {
  const args = { host: "127.0.0.1", port: 4311, seed: true, help: false, version: false };
...
async function main() {
  ...
  const { openDatabase: openDatabase2, migrate: migrate2 } = await Promise.resolve().then(() => (init_connection(), connection_exports));
  const { seedDemoCouncil: seedDemoCouncil2 } = await Promise.resolve().then(() => (init_seed(), seed_exports));
  const db = openDatabase2(config);
  migrate2(db);
  if (config.seedDemoCouncil && seedDemoCouncil2(db)) {
  ...
  if (existsSync(webOutDir)) {
    ...
    app.setNotFoundHandler((req, reply) => {
      ...
      const candidate = path2.join(webOutDir, urlPath, "index.html");
