...
function parseArgs(argv) {
  const args = { command: "serve", seed: true, help: false, version: false, json: false, options: {}, positionals: [] };
  for (let i = 0; i < argv.length; i++) {
    ...
    switch (a) {
      ...
      case "--port":
        {
          ...
          if (!value || !Number.isInteger(port) || port < 1 || port > 65535) throw new Error(`invalid port: ${value ?? "(missing)"}`);
          args.port = port;
        }
        break;
      case "-H":
      ...
      default:
        if (a.startsWith("--")) {
          const key = a.slice(2);
          if (!["council", "id", "name", "protocol", "base-url", "api-key", "provider", "model-id", "context-window", "model", "prompt", "temperature", "max-tokens", "color"].includes(key)) throw new Error(`unknown option: ${a}`);
          const next = argv[i + 1];
...
function runHeadless(args, db, packageRoot) {
  ...
  if (args.command === "provider") {
    if (args.subcommand === "list") printResult(db.prepare("SELECT id, name, protocol, base_url AS baseUrl, enabled, api_key_encrypted IS NOT NULL AS hasApiKey FROM providers ORDER BY name").all(), args.json);
      ...
      db.prepare("INSERT INTO providers (id,name,protocol,base_url,api_key_encrypted,enabled) VALUES (?,?,?,?,?,1)").run(id, name, protocol, value("base-url") ?? null, value("api-key") ? encryptSecret(value("api-key")) : null);
    ...
    if (args.subcommand === "list") printResult(db.prepare("SELECT m.id, m.model_id AS modelId, m.display_name AS displayName, p.name AS provider, m.enabled FROM models m JOIN providers p ON p.id=m.provider_id ORDER BY p.name,m.display_name").all(), args.json);
    else if (args.subcommand === "remove") {
      const id = args.positionals[0] ?? value("id");
      ...
      if (!providerId || !modelId || !displayName) throw new Error("model add requires --provider, --model-id, and --name");
      const id = randomUUID7();
      db.prepare("INSERT INTO models (id,provider_id,model_id,display_name,context_window,enabled) VALUES (?,?,?,?,?,1)").run(id, providerId, modelId, displayName, value("context-window") ? Number(value("context-window")) : null);
    ...
    if (args.subcommand === "list") printResult(db.prepare("SELECT mem.id, mem.name, mem.enabled, m.display_name AS model, p.name AS provider FROM members mem LEFT JOIN models m ON m.id=mem.model_id LEFT JOIN providers p ON p.id=m.provider_id ORDER BY mem.name").all(), args.json);
    else if (args.subcommand === "remove") {
      const id = args.positionals[0] ?? value("id");
      ...
      const id = randomUUID7();
      db.prepare("INSERT INTO members (id,name,model_id,system_prompt,temperature,max_tokens,avatar_color,enabled) VALUES (?,?,?,?,?,?,?,1)").run(id, name, modelId, value("prompt") ?? null, Number(value("temperature", "0.7")), value("max-tokens") ? Number(value("max-tokens")) : null, value("color", "#c9a227"));
    ...
    if (args.subcommand === "list") {
      printResult(db.prepare(`SELECT id, name, strategy, rounds, (SELECT COUNT(*) FROM council_members cm WHERE cm.council_id = councils.id) AS members FROM councils ORDER BY created_at`).all(), args.json);
      ...
      if (!council) throw new Error("council not found");
      const members = db.prepare(`SELECT mem.id, mem.name, m.display_name AS model, p.name AS provider FROM council_members cm JOIN members mem ON mem.id=cm.member_id LEFT JOIN models m ON m.id=mem.model_id LEFT JOIN providers p ON p.id=m.provider_id WHERE cm.council_id=? ORDER BY cm.position`).all(council.id);
    ...
    if (args.subcommand === "list") printResult(db.prepare(`SELECT s.id, COALESCE(c.name, json_extract(s.snapshot_json, '$.name')) AS council, s.status, s.topic, s.created_at AS createdAt FROM sessions s LEFT JOIN councils c ON c.id = s.council_id ORDER BY s.created_at DESC LIMIT 100`).all(), args.json);
    else if (args.subcommand === "show") {
      ...
      db.prepare("UPDATE sessions SET status='cancelled', completed_at=strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id=? AND status IN ('queued','running')").run(id);
      printResult({ ok: true }, args.json);
    } else throw new Error("supported session commands: list, show, cancel");
  } else if (args.command === "usage") {
    printResult(db.prepare(`SELECT COUNT(*) AS calls, COALESCE(SUM(prompt_tokens),0) AS promptTokens, COALESCE(SUM(completion_tokens),0) AS completionTokens, COALESCE(SUM(total_tokens),0) AS totalTokens, COALESCE(SUM(cost_usd),0) AS costUsd, SUM(CASE WHEN status='error' THEN 1 ELSE 0 END) AS errors FROM usage_events`).get(), args.json);
...
async function runLocalCouncil(args, db) {
  ...
  const members = db.prepare(`SELECT mem.id, mem.name, mem.system_prompt, mem.temperature, mem.max_tokens, m.id AS model_id, m.model_id AS model_name, m.display_name, p.id AS provider_id, p.name AS provider_name FROM council_members cm JOIN members mem ON mem.id=cm.member_id LEFT JOIN models m ON m.id=mem.model_id LEFT JOIN providers p ON p.id=m.provider_id WHERE cm.council_id=? ORDER BY cm.position`).all(council.id);
  const councilConfig = db.prepare("SELECT id, name, strategy, rounds, moderator_member_id FROM councils WHERE id=?").get(council.id);
  db.prepare(`INSERT INTO sessions (id, council_id, topic, status, snapshot_json) VALUES (?, ?, ?, 'queued', ?)`).run(sessionId, council.id, topic, JSON.stringify({ ...councilConfig, members }));
  ...
  const bus = new SessionBus2((event, sequence) => db.prepare("INSERT INTO session_events (session_id, sequence, type, payload_json) VALUES (?, ?, ?, ?)").run(event.sessionId, sequence, event.type, JSON.stringify(event)));
  const helpers = makeRunnerDbHelpers2(db);
  const runner = new SessionRunner2({ bus, recordUsage: helpers.recordUsage, insertMessage: helpers.insertMessage, loadCouncil: helpers.loadCouncil, loadModelForChat: helpers.loadModelForChat, updateSessionStatus: helpers.updateSessionStatus });
