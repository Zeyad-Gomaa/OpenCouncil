...
var init_consensus = __esm({
  "apps/server/src/engine/consensus.ts"() {
    "use strict";
    init_zod();
    ballotSchema = external_exports.object({
...
import { createReadStream, existsSync as existsSync3, realpathSync as realpathSync2, statSync as statSync2 } from "node:fs";
...
function sendExistingFile(req, reply, realWebOutDir, abs) {
  let realFile;
  let stat;
  try {
    realFile = realpathSync2(abs);
    stat = statSync2(realFile);
  } catch {
    return false;
  }
  const relative = path4.relative(realWebOutDir, realFile);
  if (relative === ".." || relative.startsWith(`..${path4.sep}`) || path4.isAbsolute(relative)) return false;
  if (!stat.isFile()) return false;
  const extension = path4.extname(realFile).toLowerCase();
  const etag = `W/"${stat.size.toString(16)}-${Math.trunc(stat.mtimeMs).toString(16)}"`;
  const immutable = relative.split(path4.sep).slice(0, 2).join("/") === "_next/static";
...
async function registerWebUi(app, webOutDir) {
  ...
  const realWebOutDir = realpathSync2(webOutDir);
