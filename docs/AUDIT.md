...
The next patch upgraded Fastify 5.12 and Next.js 16.3/React 19; added allowed-host enforcement and optional operator authentication; bounded queues, extensions, directives, provider attempts and USD reservations; and added anonymous structured peer rankings with raw ballot visibility. A later install check first upgraded, then removed, @fastify/static: the fixed static export is now served by a bounded in-process streamer, eliminating its glob/send dependency tree while preserving path containment, MIME metadata, ETags, and caching. npm then reported zero known production vulnerabilities. Vitest is pinned to 3.2.7 because 4.x broke this repository’s TSX/workspace setup. The USD guard is conservative local estimation and cannot guarantee provider billing. Automatic audit uploads remain disabled in reproducible container builds. The sections below retain the original audit record; completed items should be read in light of this follow-up.
